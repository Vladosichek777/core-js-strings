...
function orderAlphabetically(str) {
  ...
  return strArr.sort((a, b) => a.codePointAt(0) - b.codePointAt(0)).join('');
}
...
function findLongestWord(sentence) {
  const arrFromString = sentence.split(' ');
  let largestWord = 0;
  let indexTargetWord = 0;
  for (let i = 0; i < arrFromString.length; i += 1) {
    if (arrFromString[i].length > largestWord) {
      largestWord = arrFromString[i].length;
      indexTargetWord = i;
    }
  }
  return arrFromString[indexTargetWord];
}
...
function reverseWords(str) {
  ...
  const arrayWords = arrayFromString.map((item) =>
    [...item].reverse().join('')
  );
...
function invertCase(str) {
  const arraFromString = str.split('');
  const reverseCaseAr = arraFromString.map(function replaceItem(item) {
    if (item.toLowerCase() === item) {
      return item.toUpperCase();
    }
    return item.toLowerCase();
  });
  return reverseCaseAr.join('');
}
...
function extractNameFromTemplate(value) {
  ...
  return `${arrFromString[1]} ${arrFromString[2].slice(0, -1)}`;
}
